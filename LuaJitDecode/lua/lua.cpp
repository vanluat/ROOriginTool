...
Lua::Lua(const Bytecode& bytecode, const Ast& ast) : bytecode(bytecode), ast(ast) {}
	...
	if ((rawDouble & DOUBLE_EXPONENT) == DOUBLE_SPECIAL) {
		...
		if (std::stod(string) != number) {
			...
			if (std::stod(string) != number) {
				...
				assert(std::stod(string) == number, "Failed to convert number to valid string","DECODE", DEBUG_INFO);
			}
		}
...
void Lua::write_file() {
	...
	assert(WriteFile(file, writeBuffer.data(), writeBuffer.size(), &charsWritten, NULL) && !(writeBuffer.size() - charsWritten), "Failed writing to file", "DECODE", DEBUG_INFO);
	writeBuffer.clear();
	writeBuffer.shrink_to_fit();
}
